...
fn create_kitty_should_fail() {
	new_test_ext().execute_with(|| {
		// create a kitty with account #20.
		assert_noop!(
			SubstrateKitties::create_kitty(Origin::signed(20)),
...
fn transfer_non_owned_kitty_should_fail() {
	new_test_ext().execute_with(|| {
		...
		// account 10 cannot transfer a kitty with this hash.
		assert_noop!(
			SubstrateKitties::transfer(Origin::signed(10), 11, hash),
...
fn set_price_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SubstrateKitties::set_price(Origin::signed(10), hash, Some(80)));

		let kitty = SubstrateKitties::kitties(hash).expect("should found the kitty");
		assert_eq!(kitty.owner, 10);
		assert_eq!(kitty.price, Some(80));
	});
}

#[test]
fn set_price_should_fail() {
	new_test_ext().execute_with(|| {
		let hash = SubstrateKitties::kitties_owned(1)[0];

		assert_noop!(
			SubstrateKitties::set_price(Origin::signed(10), hash, Some(100)),
			Error::<Test>::NotKittyOwner
		);
	});
}
...
fn buy_kitty_should_work() {
	new_test_ext().execute_with(|| {
		...
		// acct 11 now has nothing
		assert_eq!(SubstrateKitties::kitties_owned(11).len(), 0);
		// acct 11 buy acct 10's kitty
		assert_ok!(SubstrateKitties::buy_kitty(Origin::signed(11), hash, 80));

		
		// acct 11 now has a kitty
		assert_eq!(SubstrateKitties::kitties_owned(11).len(), 1);
		let new_hash = SubstrateKitties::kitties_owned(11)[0];
		// and it has the same hash
		assert_eq!(hash, new_hash);
	});
}
...
fn buy_kitty_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SubstrateKitties::buy_kitty(Origin::signed(10), hash, 80),
			Error::<Test>::BuyerIsKittyOwner
		);
		assert_noop!(
			SubstrateKitties::buy_kitty(Origin::signed(11), hash, 80),
		...
		assert_noop!(
			SubstrateKitties::buy_kitty(Origin::signed(11), hash, 50),
			Error::<Test>::KittyBidPriceTooLow
		);
		assert_noop!(
			SubstrateKitties::buy_kitty(Origin::signed(11), hash, 200),
			Error::<Test>::NotEnoughBalance
		);
	});
}
...
fn breed_kitty_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(SubstrateKitties::create_kitty(Origin::signed(10)));
		let hash1 = SubstrateKitties::kitties_owned(10)[0];

		assert_ok!(SubstrateKitties::create_kitty(Origin::signed(10)));
		let hash2 = SubstrateKitties::kitties_owned(10)[1];

		// acct 11 now has 2 kitties
		assert_eq!(SubstrateKitties::kitties_owned(10).len(), 2);
		// acct 10 breed a kitty
		assert_ok!(SubstrateKitties::breed_kitty(Origin::signed(10), hash1, hash2));
		assert_eq!(SubstrateKitties::kitties_owned(10).len(), 3);
	});
}
...
fn breed_kitty_should_fail() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			SubstrateKitties::breed_kitty(Origin::signed(11), hash1, hash2),
			Error::<Test>::NotKittyOwner
		);
	});
}
