...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		/// The currency amout reserved when mint a kitty.
		#[pallet::constant]
		type MintReserve: Get<u32>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_kitty(origin: OriginFor<T>) -> DispatchResult {
			...
			let mint_reserve = T::MintReserve::get().into();
			// Check the minter has enough free balance
			ensure!(T::Currency::free_balance(&sender) >= mint_reserve, <Error<T>>::NotEnoughBalance);
			// Reserve
			T::Currency::reserve(&sender, mint_reserve)?;
